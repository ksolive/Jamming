...
class IOManager():
    ...
    @staticmethod
    def decode_bytes_to_audio_1(bytes_buffer):
        # 检查
    ...
    @staticmethod
    def decode_bytes_to_audio_2(bytes_buffer):
        # 检查
        if not isinstance(bytes_buffer, bytes):
            logging.error("Input buffer must be bytes!")
            return bytes_buffer
    ...
    @staticmethod
    def encode_audio_to_bytes_1(audio_clip):
        # 检查
    ...
    # 将float类型的ndarray根据扬声器采样深度编码为可以发送的bytes，双声道版本
    @staticmethod
    def encode_audio_to_bytes_2(audio_clip_left, audio_clip_right):
        # 检查
    ...
    def read_audio(in_data, frame_count, time_info, status):
        ...
        if settings.INPUT_CHANNEL == 1:
            audio_clip = IOManager.decode_bytes_to_audio_1(in_data)
            global_var.write_data.put(audio_clip)
        elif settings.INPUT_CHANNEL == 2:
            audio_clip_left, audio_clip_right = IOManager.decode_bytes_to_audio_2(
                in_data)
    ...
    def write_audio(in_data, frame_count, time_info, status):
        ...
        if settings.OUTPUT_CHANNEL == 1:
            noise_clip = Modulate.am_modulate(noise_clip, channel=1)
            out_data = IOManager.encode_audio_to_bytes_1(noise_clip)
        elif settings.OUTPUT_CHANNEL == 2:
            noise_clip_left, noise_clip_right = Modulate.am_modulate(
                noise_clip, channel=2)
            out_data = IOManager.encode_audio_to_bytes_2(
                noise_clip_left, noise_clip_right)
